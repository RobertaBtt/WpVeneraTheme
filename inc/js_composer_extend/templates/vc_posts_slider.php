...
if ( $type == 'nivo' ) {
    ...
    $el_start = '<li><div class="framed-post-w"><div class="framed-post">';
    $el_end = '</div></div></li>';
...
while ( $my_query->have_posts() ) {
    ...
    if ( $slides_content != '' && $content != '' && ( $type == ' wpb_flexslider flexslider_fade flexslider' || $type == ' wpb_flexslider flexslider_slide flexslider' ) ) {
        $description = '<div class="framed-post-caption">';
        if ($slides_title==true) $description .= '<h4 class="fp-title">' . $link_image_start . $post_title . $link_image_end .'</h4>';
        $description .= '<div class="fp-content">' . $content . '</div>';
        $description .= '</div>';
    }
