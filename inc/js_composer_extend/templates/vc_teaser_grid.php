...
while ( $my_query->have_posts() ) {
    ...
    $teasers .= '<li class="'.$isotope_item.apply_filters(VC_SHORTCODE_CUSTOM_CSS_FILTER_TAG, $li_span_class, 'vc_teaser_grid_li').$categories_css.'"><div class="framed-post">';
    // If grid layout is: Title + Thumbnail + Text
    if ( $grid_layout == 'title_thumbnail_text' ) {
        if ( $post_title )  {
            $to_filter = '<h4 class="fp-title">' . $link_title_start . $post_title . $link_title_end . '</h4>';
        ...
        if ( $thumbnail ) {
            $to_filter = '<div class="fp-featured-img">' . $link_image_start . $thumbnail . $link_image_end .'</div>';
        ...
        if ( $content ) {
            $to_filter = '<div class="fp-content">' . $content . '</div>';
...
if ( $grid_template == 'carousel' ) {
    $output .= apply_filters( 'vc_teaser_grid_carousel_arrows', '<a href="#" class="prev"><i class="icon-circle-arrow-left"></i></a> <a href="#" class="next"><i class="icon-circle-arrow-right"></i></a>' );
}
