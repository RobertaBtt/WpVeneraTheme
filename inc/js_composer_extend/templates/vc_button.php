...
$icon = ( $icon != '' && $icon != 'none' ) ? ' <i class="'.$icon.'"></i>' : '';
$position = ( $position != '' ) ? ' '.$position.'-button-position' : '';
$el_class = $this->getExtraClass($el_class);

$css_class = apply_filters(VC_SHORTCODE_CUSTOM_CSS_FILTER_TAG, 'wpb_button btn '.$color.' '.$size.' '.$el_class.$position, $this->settings['base']);

if ( $href != '' ) {
    $output = '<a class="wpb_button_a'.$a_class.' '. $css_class.'" title="'.$title.'" href="'.$href.'"'.$target.'>' . $title.$icon . '</a>';
} else {
    $output .= '<button class="'.$css_class.'">'.$title.$icon.'</button>';
